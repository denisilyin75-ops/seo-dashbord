...
1. **Один пользователь** — без сложного auth, multi-tenancy
2. **SQLite** — один файл, простой бэкап
3. **Не трогать WordPress core** — только REST API
4. **Каждый сайт = продаваемый актив** — не привязываться к конкретному
5. **Graceful degradation** — если GA4/GSC не подключён, показывать placeholder
6. **AI-команды idempotent**
7. **Секреты только в .env**
8. **Логировать всё** — AI команды, деплои, sync
